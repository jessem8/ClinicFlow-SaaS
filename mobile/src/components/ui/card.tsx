...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { variant?: "default" | "elevated" | "interactive" | "glass" }>(
  ({ className, variant = "default", ...props }, ref) => {
    const variants = {
      ...
      interactive: "rounded-xl border bg-card text-card-foreground shadow-sm hover:shadow-lg transition-all duration-200 hover:border-primary/20 cursor-pointer",
      glass: "rounded-xl bg-card/80 backdrop-blur-md border border-border/50 text-card-foreground",
    };
